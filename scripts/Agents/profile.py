...
class Profile:
    ...
    def __init__(self, name: str, traits: Dict[str, float], occupation: str, 
        ...
        self.status = {
            # 按要求移除了饥饿和能量
            "mood": 50,  # 0-100, 0 表示非常悲伤, 100 表示非常开心
            "health": 100  # 0-100, 0 表示非常生病
        }
        
    def update_status(self, status_type: str, value: int) -> None:
        """
        Update agent's status value, ensuring it stays within valid range.
        
        Args:
            status_type: The status type to update (e.g., 'mood', 'health')
            value: The value to add (can be positive or negative)
        """
        if status_type in self.status:
            # 记录旧值
            old_value = self.status[status_type]
            ...
            # Actually update the status value
            self.status[status_type] = new_value
            
            # Debug output to confirm the change
            print(f"Status '{status_type}' changed: {old_value} -> {new_value} (change: {value})")
        else:
            print(f"Warning: Attempted to update unknown status type '{status_type}'")
            # 自动添加新状态类型
            if value >= 0:
                self.status[status_type] = min(100, value)
                print(f"Created new status '{status_type}' with value {self.status[status_type]}")
            else:
                self.status[status_type] = 0
                print(f"Created new status '{status_type}' with value 0 (attempted negative value)")
